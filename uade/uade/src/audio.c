...
/* Scope capture ring buffers for per-channel waveform visualization */
#define SCOPE_BUFFER_SIZE 1024
#define SCOPE_BUFFER_MASK (SCOPE_BUFFER_SIZE - 1)
static float scope_buffer[4][SCOPE_BUFFER_SIZE];
static unsigned int scope_write_pos[4];
static int scope_enabled;
...
static void sample16s_handler (void)
{
    ...
    if (scope_enabled) {
	for (i = 0; i < 4; i++) {
	    scope_buffer[i][scope_write_pos[i] & SCOPE_BUFFER_MASK] = (float)output[i] / 8192.0f;
	    scope_write_pos[i]++;
	}
    }
...
{
    scope_enabled = enable;
    if (enable) {
	memset(scope_buffer, 0, sizeof(scope_buffer));
	memset(scope_write_pos, 0, sizeof(scope_write_pos));
    }
}

unsigned int audio_scope_get_data(int channel, float *buffer, unsigned int num_samples)
{
    unsigned int i, wp, read_pos;

    if (channel < 0 || channel >= 4 || buffer == NULL)
	return 0;
    if (num_samples > SCOPE_BUFFER_SIZE)
	num_samples = SCOPE_BUFFER_SIZE;

    wp = scope_write_pos[channel];
    read_pos = (wp - num_samples) & SCOPE_BUFFER_MASK;

    for (i = 0; i < num_samples; i++) {
	buffer[i] = scope_buffer[channel][read_pos];
	read_pos = (read_pos + 1) & SCOPE_BUFFER_MASK;
    }
