...
void Analyze::dump() {
    ...
    for (SeqTraitsMap::iterator it = sndSeqCmdMap.begin(); it != sndSeqCmdMap.end(); ++it) {
        cout << hexB(it->first) << " : ";
        SeqTraits p = it->second;
        std::for_each(p.valuesUsed.begin(), p.valuesUsed.end(), [](ubyte c) { cout << hexB(c) << ' '; });
        cout << endl;
    }
    cout << endl;

    cout << "Volume sequence commands used:" << endl;
    for (SeqTraitsMap::iterator it = volSeqCmdMap.begin(); it != volSeqCmdMap.end(); ++it) {
        cout << hexB(it->first) << " : ";
        SeqTraits p = it->second;
        std::for_each(p.valuesUsed.begin(), p.valuesUsed.end(), [](ubyte c) { cout << hexB(c) << ' '; });
        cout << endl;
    ...
    std::for_each(samplesSet.begin(), samplesSet.end(), [](ubyte c) { cout << hex << (int)c << ' '; });
    cout << endl;

    cout << "Vibrato amplitudes: ";
    std::for_each(vibratoAmplSet.begin(), vibratoAmplSet.end(), [](ubyte c) { cout << hexB(c) << ' '; });
    cout << endl;
    cout << "Vibrato speeds: ";
    std::for_each(vibratoSpeedSet.begin(), vibratoSpeedSet.end(), [](ubyte c) { cout << hexB(c) << ' '; });
    cout << endl;

    cout << "Negative vibrato speed : " << (int)usesNegVibSpeed() << endl;

    cout << "Sound sequence transpose/pitch values used:" << endl;
    for (SeqTraitsMap::iterator it = seqTransMap.begin(); it != seqTransMap.end(); ++it) {
        cout << hexB(it->first) << " : ";
        SeqTraits p = it->second;
        std::for_each(p.valuesUsed.begin(), p.valuesUsed.end(), [](ubyte c) { cout << hexB(c) << ' '; });
        cout << endl;
    ...
    for (SeqTraitsMap::iterator it = sndSeqCmdMap.begin(); it != sndSeqCmdMap.end(); ++it) {
        ...
        if (p.valuesUsed.count(0xE7) == 1) {
            ...
            for (udword i = 0; i < (seqOffsEnd - seqOffs); i++) {
                if (decoder->fcBuf[seqOffs + i] == 0xE0 || decoder->fcBuf[seqOffs + i] == 0xE1) {
                    return true;
                }
            }
        }
