...
bool HippelDecoder::COSO_init(int songNumber) {
    ...
    if (readBEudword(fcBuf, h + 0xc) == 0 || readBEudword(fcBuf, h + 0x10) == 0 || readBEudword(fcBuf, h + 0x14) == 0
        || readBEudword(fcBuf, h + 0x1c) == 0 || readBEudword(fcBuf, h + 0x18) >= input.len) {
        return false;
    }
...
#ifdef DEBUG
        cout << "WARNING: mismatch between offset sample data 0x" << hex << offsets.sampleData
             << " and calculated via number of samples 0x"
             << (offsets.sampleHeaders + stats.samples * traits.sampleStructSize) << endl;
#endif
    }
    ...
    for (ubyte seq = 0; seq < stats.sndSeqs; seq++) {
        ...
        if (stats.samples < 0xe5 && fcBuf[seqOffs] == 0xe2 && fcBuf[seqOffs + 1] >= 0xe5) {
            return false;
        }
    }
    ...
    for (int sam = 0; sam < stats.samples; sam++) {
        ...
        if (samples[sam].repLen == 1) {
            fcBuf[samples[sam].startOffs + samples[sam].repOffs]
                = fcBuf[samples[sam].startOffs + samples[sam].repOffs + 1] = 0;
        }
