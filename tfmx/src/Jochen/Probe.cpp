...
bool HippelDecoder::TFMX_findPortamentoCode() {
    ...
    do {
        ...
        if (r != (input.buf + input.len)) {
            ...
            if ((readBEuword(fcBuf, o - 6) == 0x4a01 || readBEuword(fcBuf, o - 4) == 0x4a01)
                ...
                // MOVE.l D2,$00??(A0)
                readBEuword(fcBuf, o + 10) == 0x2142 && readBEuword(fcBuf, o + 12) == voiceVar &&
                // SWAP D2
...
bool HippelDecoder::TFMX_7V_findPlayer(const ubyte* buf, udword len) {
    ...
    const ubyte seqJmp[] = {
        0x04, 0x40, 0x00, 0xe0, 0xd0, 0x40, 0x30, 0x3b, 0x00, 0x06, 0x4e, 0xfb, 0x00 // ,0x?? <--
    };
    ...
    do {
        ...
        if (r != (buf + len)) {
            ...
            if (sBuf[o] == 0x14 && readBEuword(sBuf, o + 1 + 2) == 0x16 && readBEuword(sBuf, o + 1 + 4) == 0x16
...
bool HippelDecoder::TFMX_4V_maybe() {
    ...
    if (probeOffset >= input.len || readBEuword(fcBuf, probeOffset) != 0 || readBEuword(fcBuf, probeOffset + 2) != 0
...
bool HippelDecoder::TFMX_7V_maybe() {
    ...
    udword probeOffset
        = offsets.trackTable + stats.trackSteps * TFMX_7V_TRACKTAB_STEP_SIZE + stats.songs * TFMX_7V_SONGTAB_ENTRY_SIZE;
    if (probeOffset >= input.len || readBEudword(fcBuf, probeOffset) != 0
...
bool HippelDecoder::COSO_findPlayer(const ubyte* buf, udword len) {
    const ubyte pattCode[] = { 0x0c, 0x00, 0x00, 0xfe, 0x66 };
    ...
    do {
        ...
        if (r != (buf + len)) {
            o = (udword)(r - buf);
            if (readBEuword(sBuf, o + 6) == 0x1151 && readBEuword(sBuf, o + 0xa) == 0x1159
...
bool HippelDecoder::findTag(const ubyte* buf, udword len, const std::string tag, udword& location) {
