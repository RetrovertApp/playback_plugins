...
void HippelDecoder::reset() {
    ...
    for (ubyte v = 0; v < stats.voices; v++) {
        ...
        voiceVars[v].trackPos = voiceVars[v].pattPos = 0;
        voiceVars[v].volSlideSpeed = voiceVars[v].volSlideTime = voiceVars[v].volSlideDelayFlag
            = voiceVars[v].volSustainTime = 0;
        voiceVars[v].vibSpeed = voiceVars[v].vibDelay = voiceVars[v].vibAmpl = voiceVars[v].vibCurOffs
            = voiceVars[v].vibFlag = 0;
        voiceVars[v].pitchBendSpeed = voiceVars[v].pitchBendTime = voiceVars[v].pitchBendDelayFlag = 0;
        voiceVars[v].transpose = voiceVars[v].soundTranspose = voiceVars[v].seqTranspose = 0;
        voiceVars[v].portaSpeed = voiceVars[v].portaOffs = voiceVars[v].portaDelayFlag = 0;
        voiceVars[v].volSeq = offsets.silence;
        voiceVars[v].sndSeq = offsets.silence + 1;
        voiceVars[v].volSeqPos = voiceVars[v].currentVolSeq = voiceVars[v].currentSndSeq = voiceVars[v].sndSeqPos = 0;
        voiceVars[v].sndModSustainTime = 0;
        voiceVars[v].pattVal1 = voiceVars[v].pattVal2 = 0;
        voiceVars[v].outputPeriod = voiceVars[v].outputVolume = voiceVars[v].volume = 0;
        voiceVars[v].voiceVol = 100;
        voiceVars[v].volRandomThreshold = voiceVars[v].volRandomCurrent = 0;
...
void HippelDecoder::clearFormat() {
    ...
    traits.compressed = traits.vibScaling = traits.vibSpeedFlag = traits.sndSeqGoto = traits.sndSeqToTrans
        = traits.skipToWaveMod = traits.checkSSMPtag = traits.foundSSMP = traits.volSeqSnd80 = traits.volSeqEA
        = traits.randomVolume = traits.voiceVolume = traits.portaIsBit6 = traits.porta40SetSnd = traits.porta80SetSnd
        = traits.isSMOD = traits.lowerPeriods = traits.portaWeaker = traits.blacklisted = false;
    ...
    ModFuncPtr defaultSndModFuncs[(0xea - 0xe0) + 1]
        = { &HippelDecoder::TFMX_sndSeq_E0, &HippelDecoder::TFMX_sndSeq_E1,     &HippelDecoder::TFMX_sndSeq_E2,
            &HippelDecoder::TFMX_sndSeq_E3, &HippelDecoder::TFMX_sndSeq_E4,     &HippelDecoder::TFMX_sndSeq_E5,
            &HippelDecoder::TFMX_sndSeq_E6, &HippelDecoder::TFMX_sndSeq_E7,     &HippelDecoder::TFMX_sndSeq_E8,
            &HippelDecoder::TFMX_sndSeq_E9, &HippelDecoder::TFMX_sndSeq_EA_skip };
        ...
        if (!songEnd || loopMode) {
            ...
            if (voiceVars[v].repeatDelay != 0) {
                if (--voiceVars[v].repeatDelay == 1) {
                    voiceVars[v].repeatDelay = 0;
                    voiceVars[v].ch->paula.start = voiceVars[v].pSampleStart + voiceVars[v].repeatOffset;
...
void HippelDecoder::dumpModule() {
    ...
    cout << "Volume modulation sequences: 0x" << hexB(stats.volSeqs) << " at 0x" << tohex(offsets.volModSeqs) << endl;
    if (!traits.compressed) {
    ...
    cout << "Sound modulation sequences: 0x" << hexB(stats.sndSeqs) << " at 0x" << tohex(offsets.sndModSeqs) << endl;
    if (!traits.compressed) {
    ...
    if (!traits.compressed) {
        dumpBlocks(fcBuf, offsets.patterns, stats.patterns * traits.patternSize, traits.patternSize);
    } else {
    ...
    for (int sam = 0; sam < stats.samples; sam++) {
        cout << "0x" << hexB(sam) << ": " << tohex(samples[sam].startOffs) << " " << hexW(samples[sam].len) << " "
             << hexW(samples[sam].repOffs) << " " << hexW(samples[sam].repLen) << " " << endl;
    }
#endif // DEBUG
}
...
const uword HippelDecoder::periodsLower[0x40] = {
    ...
    0x00d6, 0x00ca, 0x00be, 0x00b4, 0x00aa, 0x00a0, 0x0097, 0x008f, 0x0087, 0x007f, 0x0078, 0x0071,
    // +0x30
    0x006b, 0x0065, 0x005f, 0x005a, 0x0055, 0x0050, 0x004c, 0x0048, 0x0044, 0x0040, 0x003c, 0x0039,
    // +0x3c
