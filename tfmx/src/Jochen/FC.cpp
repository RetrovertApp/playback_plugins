...
bool HippelDecoder::FC_init(int songNumber) {
    ...
    for (int sam = 0; sam < 10; sam++) {
        ...
        if (!traits.isSMOD) {
            ...
            if (SSMP_HEX != readBEudword(fcBuf, pos) && samples[sam].repLen == 1) {
...
void HippelDecoder::FC_nextNote(VoiceVars& voiceX) {
    ...
    if (voiceX.pattPos == traits.patternSize || (!traits.isSMOD && fcBuf[pattOffs] == FC_PATTERN_BREAK)) {
        ...
        if (voiceX.voiceNum == 0) {
            ...
            cout << " | " << hexW(voiceX.trackStart + voiceX.trackPos) << " of " << hexW(voiceX.trackStart) << " to "
                 << hexW(voiceX.trackEnd) << endl;
