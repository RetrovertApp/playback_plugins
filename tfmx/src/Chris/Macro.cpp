...
void TFMXDecoder::dumpMacros() {
    ...
    do {
        ...
        if ((macroStart < prevMacroStart) || (macroStart == offsets.header) || (macroStart >= offsets.sampleData)) {
        ...
        if ((macroEnd < macroStart) || (macroEnd == getPattOffset(0)) || (macroEnd == offsets.header)
        ...
        cout << "Macro 0x" << hexB(macro) << " at 0x" << tohex(macroStart) << " to 0x" << tohex(macroEnd) << endl;
        int step = 0;
        while (macroStart < macroEnd) {
            int cmd = 0x3f & pBuf[macroStart];
            cout << "  " << hexW(step) << ' ' << hexB(cmd) << ' ' << hex << std::setw(6) << std::setfill('0')
                 << (int)(0x00ffffff & readBEudword(pBuf, macroStart)) << ' ' << MacroDefs[cmd]->text << endl;
            if (cmd == 7) {
                break;
            }
            macroStart += 4;
...
void TFMXDecoder::processMacroMain(VoiceVars& voice) {
    ...
    do {
        ...
        cout << std::hex << std::setw(1) << '[' << (int)voice.voiceNum << "] " << tohex(voice.macro.offset) << '/'
             << hexW(voice.macro.step) << ' ' << hexB(command) << ' ' << std::setw(6)
             << (int)(0x00ffffff & makeDword(0, cmd.bb, cmd.cd, cmd.ee));
#endif
...
void TFMXDecoder::macroFunc_28(VoiceVars& voice) { // SID op1
    ...
    voice.sid.op1.interDelta
        = makeWord(cmd.ee, voice.sid.op1.interDelta & 0xff); // change the high-byte, keep the low-byte
