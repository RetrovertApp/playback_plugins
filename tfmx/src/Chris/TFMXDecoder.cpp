...
void TFMXDecoder::setPath(std::string path) {
    ...
    if (found != std::string::npos) {
        ...
        if (fname.compare(0, mdatLC.length(), mdatLC) == 0 || fname.compare(0, mdatUC.length(), mdatUC) == 0) {
...
#if defined(DEBUG)
    cout << "Sequencer: " << dec << (int)sequencer.step.first << " to " << (int)sequencer.step.last << " / speed "
         << (int)admin.speed << endl;
#endif
    processTrackStep();
}
...
void TFMXDecoder::adjustTraitsPost() {
    ...
    if (trackCmdUsed[3] || input.versionHint == 3) { // from TFHD
        voices = 8;
...
void TFMXDecoder::dumpModule() {
    ...
    cout << "Track Cmd used:" << endl << "01234" << endl;
    for (int m = 0; m <= TRACK_CMD_MAX; m++) {
    ...
    cout << "Pattern Cmd used:" << endl << "0123456789abcdef" << endl;
    for (int m = 0; m < 16; m++) {
...
int TFMXDecoder::run() {
    ...
    if (!songEnd || loopMode) {
        if (--admin.count < 0) {
            ...
            patternTickCounter++;       // increment for position mapping
            do {
                sequencer.step.next = false;
                int countInactive = 0;
                int countInfinite = 0;
...
void TFMXDecoder::noteCmd() {
    ...
    if (cmd.aa == 0xfc) {        // lock note
        ...
        v.note = cmd.aa;
        v.keyUp = false;
        v.macroNum = cmd.bb & 0x7f;
        v.macro.offset = getMacroOffset(v.macroNum);
        v.macro.step = 0;
        v.macro.wait = 0;
        v.macro.loop = 0xff;
        v.macro.skip = false;
...
const uword TFMXDecoder::periods[0xd + 0x40]
    = { 0x0d5c,
        // -0xf4 extra octave
        0x0c9c, 0x0be8, 0x0b3c, 0x0a9a, 0x0a02, 0x0a02, 0x0972, 0x08ea, 0x086a, 0x07f2, 0x0780, 0x0718,
        // +0 standard octaves
        0x06ae, 0x064e, 0x05f4, 0x059e, 0x054d, 0x0501, 0x04b9, 0x0475, 0x0435, 0x03f9, 0x03c0, 0x038c,
        // +0x0c
        0x0358, 0x032a, 0x02fc, 0x02d0, 0x02a8, 0x0282, 0x025e, 0x023b, 0x021b, 0x01fd, 0x01e0, 0x01c6,
        // +0x18
        0x01ac, 0x0194, 0x017d, 0x0168, 0x0154, 0x0140, 0x012f, 0x011e, 0x010e, 0x00fe, 0x00f0, 0x00e3,
        // +0x24
