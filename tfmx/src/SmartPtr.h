  ...
  public:
    ...
    virtual ~smartPtrBase() {
        if (doFree && (bufBegin != 0)) {
            // Note: Old MSVC workaround removed. The void* cast is undefined behavior
            // and modern compilers (including clang-cl) handle this correctly.
            delete[] bufBegin;
        }
    }
    ...
    virtual ulong tellLength() {
        return bufLen;
    }
    virtual ulong tellPos() {
        return (ulong)(pBufCurrent - bufBegin);
    }
...
template <class T> class smartPtr : public smartPtrBase<T> {
  public:
    smartPtr(T* buffer, ulong bufferLen, bool bufOwner = false) : smartPtrBase<T>(buffer, bufferLen, bufOwner) {}

    smartPtr() : smartPtrBase<T>(0, 0) {}
