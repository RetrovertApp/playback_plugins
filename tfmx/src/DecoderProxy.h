...
class DecoderProxy {
  public:
    ...
    // Pattern display API
    bool buildPatternDisplay();
    int getPatternDisplayNumTracks();
    uint32_t getPatternDisplayTrackRows(int track);
    bool getPatternDisplayRow(int track, uint32_t row, uint8_t* type, uint8_t* note, uint8_t* macro, uint8_t* volume,
                              int8_t* detune, uint8_t* channel, uint16_t* wait, uint32_t* tick);
    uint32_t getPatternTickCounter();
    int findPatternDisplayRowForTick(int track, uint32_t tick);
