...
/* ============================================================================
   Pattern Display API - per-track unrolled pattern data for visualization
   ============================================================================ */

/* Build the pattern display data for the current song.
   Must be called after tfmxdec_init.
   Returns: 1 on success, 0 on failure */
int tfmxdec_build_pattern_display(void* decoder);

/* Get number of tracks in the pattern display.
   Returns: number of tracks (typically 4-8) */
int tfmxdec_get_pattern_display_num_tracks(void* decoder);

/* Get number of rows for a specific track.
   track: 0-based track index
   Returns: number of rows, or 0 if invalid */
uint32_t tfmxdec_get_pattern_display_track_rows(void* decoder, int track);

/* Get a row from a track's pattern display.
   track: 0-based track index
   row: 0-based row index
   type: 0=note, 1=command (output, can be NULL)
   note: note value 0-63 or command type (output, can be NULL)
   macro: macro number or command parameter (output, can be NULL)
   volume: channel volume 0-15 (output, can be NULL)
   detune: detune value (output, can be NULL)
   channel: destination channel (output, can be NULL)
   wait: wait ticks (output, can be NULL)
   tick: cumulative tick position (output, can be NULL)
   Returns: 1 on success, 0 on failure */
int tfmxdec_get_pattern_display_row(void* decoder, int track, uint32_t row, uint8_t* type, uint8_t* note,
                                    uint8_t* macro, uint8_t* volume, int8_t* detune, uint8_t* channel, uint16_t* wait,
                                    uint32_t* tick);

/* Get current pattern tick counter (for position mapping).
   Returns: current tick value */
uint32_t tfmxdec_get_pattern_tick(void* decoder);

/* Find the row index for a given tick value in a track (binary search).
   track: 0-based track index
   tick: tick value to search for
   Returns: row index (last row with tick <= given tick), or -1 if not found */
int tfmxdec_find_pattern_display_row_for_tick(void* decoder, int track, uint32_t tick);

/* ============================================================================
   Scope Capture API - per-voice audio waveform capture for visualization
   ============================================================================ */

/* Enable or disable scope capture for waveform visualization.
   When enabled, the mixer captures per-voice audio samples into ring buffers.
   enable: 1 = enable, 0 = disable */
void tfmxdec_enable_scope_capture(void* decoder, int enable);

/* Check if scope capture is currently enabled.
   Returns: 1 if enabled, 0 if disabled */
int tfmxdec_is_scope_capture_enabled(void* decoder);

/* Get scope data (audio waveform samples) for a specific voice.
   voice: 0-based voice index (0-7 for TFMX)
   buffer: output buffer for samples (normalized floats in [-1.0, 1.0] range)
   num_samples: maximum number of samples to retrieve (up to 1024)
   Returns: actual number of samples written to buffer */
uint32_t tfmxdec_get_scope_data(void* decoder, int voice, float* buffer, uint32_t num_samples);
