...
class LamePaulaMixer {
  public:
    ...
    bool isScopeCaptureEnabled() const;
    uint32_t getScopeData(int voice, float* buffer, uint32_t numSamples);
  ...
  protected:
    ...
    static const int SCOPE_BUFFER_SIZE = 1024; // Power of 2 for efficient wrapping

    LamePaulaVoice* pVoice[MAX_VOICES];
    ubyte voiceVol[MAX_VOICES];
    bool muted[MAX_VOICES];

    // Per-voice scope capture ring buffers
    float m_scopeBuffer[MAX_VOICES][SCOPE_BUFFER_SIZE];
    int m_scopeWritePos[MAX_VOICES];
    bool m_scopeEnabled;
    ...
    enum VoicePos { LEFT, RIGHT };
    // Paula voices 0 and 3 are connected to left output, 1 and 2 to right output.
    VoicePos panningPos[MAX_VOICES] = { LEFT, RIGHT, RIGHT, LEFT, LEFT, LEFT, LEFT };
