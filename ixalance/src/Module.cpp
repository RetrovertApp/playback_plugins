...
namespace IXS {
...
#elif defined(LINUX)
  uint IXS__Module__loadSongFromCache(FileSFXI *smplFile, Module *module, byte *itFileHead);
#else
  ...
  int __thiscall loadIxsInputFileData(Module *module, std::string filename, byte *itFileHead, byte *dataBuf1, uint lenBuf1,
    ...
    if(IXS__Module__loadSongFromCache(filename, module, itFileHead, smplFile)) {
...
#elif defined(LINUX)
  // LINUX version: generate sample cache entirely in memory (no file I/O)
  static BufSFXI *linuxCacheFileBuf = nullptr;

  int __thiscall loadIxsInputFileData(Module *module, std::string filename, byte *itFileHead, byte *dataBuf1, uint lenBuf1,
                                      byte *dataBuf2, uint lenBuf2, float outputVolume, FileSFXI *smplFile, FileSFXI *tmpFile) {
    FileSFXI *cacheSmplFile = smplFile;

    if (cacheSmplFile == nullptr) {
      // Need to generate sample cache in memory
      if (!linuxCacheFileBuf) {
        linuxCacheFileBuf = (BufSFXI *) malloc(0x1e00000); // 32MB
      } else {
        linuxCacheFileBuf->magic_0x0 = 0;  // trigger reset
      }
      ...
      if (tmpFile != nullptr) {
        ...
        destSFXI = IXS__FileSFXI__newFileSFXI_00413c20(linuxCacheFileBuf);
      }
      IXS__WAVEGEN__createSampleCacheFile((char*)filename.c_str(), dataBuf1, dataBuf2, destSFXI);
      cacheSmplFile = destSFXI;
    }

    if(IXS__Module__loadSongFromCache(cacheSmplFile, module, itFileHead)) {
      fprintf(stderr, "error: invalid IT file\n");
      return 1;
    }
...
#elif defined(LINUX)
  // LINUX: load from FileSFXI directly (no file I/O)
  uint IXS__Module__loadSongFromCache(FileSFXI *smplFile, Module *module, byte *itFileHead) {
    uint retVal = 0;
    if(IXS__Module__readImpulseTrackerData_0x408450(module, itFileHead, smplFile)) {
      retVal = 1;
...
#else
  // EMSCRIPTEN: load from memory buffer
  uint IXS__Module__loadSongFromCache(byte *cacheData, Module *module, byte *itFileHead) {
...
//      IXS__Module__copyFromInputFile_004092f0(module, module, 0xc0);
    ...
    module->insPtrArray_0xcc = (ITInstrument **) malloc(sizeof(ITInstrument*) * (uint) itHeader.InsNum_0x22);
    if (insNum != 0) {
      int i = 0;
      do {
    ...
    module->smplHeadPtrArr0_0xd0 = (ITSample **) malloc(sizeof(ITSample*) * (uint) itHeader.SmpNum_0x24);
    byte **smplDataPtrArray = (byte **) malloc(sizeof(byte*) * (uint) itHeader.SmpNum_0x24);
    ...
    module->patHeadPtrArray_0xd8 = (ITPatternHead **) malloc((uint) itHeader.PatNum_0x26 << 2);
    ushort patNum = itHeader.PatNum_0x26;
    module->patDataPtrArray_0xdc = (byte **) malloc((uint) itHeader.PatNum_0x26 << 2);;
    module->lastOrder_0xfaec = -1;
    if (patNum != 0) {
      int i = 0;
      do {
  ...
  byte *__thiscall IXS__Module__outputTrackerFile_00409430(Module *module, uint unused) {
    ...
    if ((module->impulseHeader_0x0).InsNum_0x22 != 0) {
      do {
        ...
        do {
          if ((pbVar5[1] != sample) && ((uint) pbVar5[0] != i)) {
    ...
    size_t len = (byte *) module->writeBufferPosPtr_0xfae8 - (byte *) writeBufferPtr;
    module->writeBufferPosPtr_0xfae8 = (byte *) writeBufferPtr;
