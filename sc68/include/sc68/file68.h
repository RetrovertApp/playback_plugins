 ...
 * @retval  0     on success
 */
int file68_tag_enum(const disk68_t* mb, int track, int idx, const char** key, const char** val);
 ...
 * @param  gzip     gzip compression level [0:no-gzip, 1..9 or -1]
 */
int file68_save_uri(const char* uri, const disk68_t* mb, int version, int gzip);
 ...
 * @param  gzip     gzip compression level [0:no-gzip, 1..9 or -1]
 */
int file68_save_mem(const char* buffer, int len, const disk68_t* mb, int version, int gzip);
