	...
	for( j = 0; j < length; j++ )
	{
		...
		if (! chip->NgpFlags)
		{
			...
			for ( i = 0; i <= 3; ++i )
			{
				...
				scope_write(&chip->scope, i, chnOut, 2000.0f);
...
{
	scope_set_enabled(&((SN76489_Context*)chip)->scope, enabled);
}

UINT8 sn76489_get_scope_enabled(void *chip)
{
	return scope_get_enabled(&((SN76489_Context*)chip)->scope);
}

const float* sn76489_get_scope_buffer(void *chip, UINT8 channel, UINT32* out_write_pos)
{
	return scope_get_buffer(&((SN76489_Context*)chip)->scope, channel, out_write_pos);
}
