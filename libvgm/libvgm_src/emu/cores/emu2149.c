...
update_output (EPSG * psg)
{
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    scope_write(&psg->scope, i, psg->ch_out[i], 6000.0f);
...
/* Scope capture API */
void EPSG_set_scope_enabled(EPSG *psg, UINT8 enabled)
{
  if (psg == NULL)
    return;
...
UINT8 EPSG_get_scope_enabled(EPSG *psg)
{
  if (psg == NULL)
    return 0;
...
const float* EPSG_get_scope_buffer(EPSG *psg, UINT8 channel, UINT32* out_write_pos)
{
  if (psg == NULL)
    return NULL;
