...
static void sn76496_update(void* param, UINT32 samples, DEV_SMPL** outputs)
{
	...
	//		R->current_clock = R->clock_divider - 1;
		...
		if (! R->NgpFlags)
		{
			for (i = 0; i < 4; i ++)
			{
				INT32 chnOut;  // Per-channel output for scope capture
				...
				// Scope capture - capture per-channel output
				scope_write(&R->scope, i, chnOut, 6000.0f);
			}
		}
		else
		{
...
{
	scope_set_enabled(&((sn76496_state*)chip)->scope, enabled);
}

UINT8 sn76496_get_scope_enabled(void *chip)
{
	return scope_get_enabled(&((sn76496_state*)chip)->scope);
}

const float* sn76496_get_scope_buffer(void *chip, UINT8 channel, UINT32* out_write_pos)
{
	return scope_get_buffer(&((sn76496_state*)chip)->scope, channel, out_write_pos);
}
