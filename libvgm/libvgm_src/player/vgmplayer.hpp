...
public:
	...
	// Scope capture interface
	void SetScopeEnabled(bool enabled);
	bool GetScopeEnabled(void) const;
	UINT8 GetScopeChannelCount(void) const;
	const char* GetScopeChannelName(UINT8 channel) const;
	UINT32 GetScopeData(UINT8 channel, float* buffer, UINT32 maxSamples) const;
