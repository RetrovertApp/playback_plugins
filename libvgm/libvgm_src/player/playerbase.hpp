...
public:
	...
	// Scope capture interface (optional, default implementations return empty)
	virtual void SetScopeEnabled(bool enabled);
	virtual bool GetScopeEnabled(void) const;
	virtual UINT8 GetScopeChannelCount(void) const;
	virtual const char* GetScopeChannelName(UINT8 channel) const;
	virtual UINT32 GetScopeData(UINT8 channel, float* buffer, UINT32 maxSamples) const;
