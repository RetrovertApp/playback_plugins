...
void PlayerBase::SetScopeEnabled(bool enabled)
{
	(void)enabled;
	// Default: no-op
}

bool PlayerBase::GetScopeEnabled(void) const
{
	return false;
}

UINT8 PlayerBase::GetScopeChannelCount(void) const
{
	return 0;
}

const char* PlayerBase::GetScopeChannelName(UINT8 channel) const
{
	(void)channel;
	return NULL;
}

UINT32 PlayerBase::GetScopeData(UINT8 channel, float* buffer, UINT32 maxSamples) const
{
	(void)channel;
	(void)buffer;
	(void)maxSamples;
