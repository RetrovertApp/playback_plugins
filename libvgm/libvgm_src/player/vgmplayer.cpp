...
#include "../emu/cores/sn76489.h"		// for Maxim PSG scope capture
#include "../emu/cores/sn76496.h"		// for MAME PSG scope capture
#include "../emu/cores/2612intf.h"
#include "../emu/cores/ym2612.h"		// for Gens core scope capture
#include "../emu/cores/fmopn.h"			// for MAME core scope capture
#include "../emu/scope_capture.h"		// for SCOPE_BUFFER_SIZE
#include "../emu/cores/segapcm.h"		// for SEGAPCM_CFG
#include "../emu/cores/ayintf.h"		// for AY8910_CFG
#include "../emu/cores/ay8910.h"		// for MAME AY scope capture
#include "../emu/cores/emu2149_private.h"	// for EMU2149 scope capture
...
void VGMPlayer::SetScopeEnabled(bool enabled)
{
	_scopeEnabled = enabled;

	// Enable/disable scope capture on YM2612 chips (GPGX/MAME and Gens cores supported)
	for (UINT8 chipID = 0; chipID < 2; chipID++)
	{
		CHIP_DEVICE* dev = GetDevicePtr(0x02, chipID);  // 0x02 = YM2612 VGM chip type
		if (dev != NULL && dev->base.defInf.dataPtr != NULL && dev->base.defInf.devDef != NULL)
		{
			UINT32 coreID = dev->base.defInf.devDef->coreID;
			if (coreID == FCC_GENS)
			{
				ym2612_* ym = (ym2612_*)dev->base.defInf.dataPtr;
				YM2612_SetScopeEnabled(ym, enabled ? 1 : 0);
			}
			else if (coreID == FCC_GPGX)  // MAME/GPGX core uses FCC_GPGX
			{
				ym2612_set_scope_enabled(dev->base.defInf.dataPtr, enabled ? 1 : 0);
			}
		}
	}

	// Enable/disable scope capture on SN76489/PSG chips (MAME and Maxim cores supported)
	for (UINT8 chipID = 0; chipID < 2; chipID++)
	{
		CHIP_DEVICE* dev = GetDevicePtr(0x00, chipID);  // 0x00 = SN76496/PSG VGM chip type
		if (dev != NULL && dev->base.defInf.dataPtr != NULL && dev->base.defInf.devDef != NULL)
		{
			UINT32 coreID = dev->base.defInf.devDef->coreID;
			if (coreID == FCC_MAXM)  // Maxim core
			{
				sn76489_set_scope_enabled(dev->base.defInf.dataPtr, enabled ? 1 : 0);
			}
			else if (coreID == FCC_MAME)  // MAME core
			{
				sn76496_set_scope_enabled(dev->base.defInf.dataPtr, enabled ? 1 : 0);
			}
		}
	}

	// Enable/disable scope capture on AY8910 chips (MAME and EMU2149 cores supported)
	for (UINT8 chipID = 0; chipID < 2; chipID++)
	{
		CHIP_DEVICE* dev = GetDevicePtr(0x12, chipID);  // 0x12 = AY8910 VGM chip type
		if (dev != NULL && dev->base.defInf.dataPtr != NULL && dev->base.defInf.devDef != NULL)
		{
			UINT32 coreID = dev->base.defInf.devDef->coreID;
			if (coreID == FCC_MAME)  // MAME core
			{
				ay8910_set_scope_enabled(dev->base.defInf.dataPtr, enabled ? 1 : 0);
			}
			else if (coreID == FCC_EMU_)  // EMU2149 core
			{
				EPSG_set_scope_enabled((EPSG*)dev->base.defInf.dataPtr, enabled ? 1 : 0);
			}
		}
	}
}

bool VGMPlayer::GetScopeEnabled(void) const
{
	return _scopeEnabled;
}

UINT8 VGMPlayer::GetScopeChannelCount(void) const
{
	UINT8 count = 0;

	// Count YM2612 channels (6 per chip) - GPGX/MAME and Gens cores support scope
	for (UINT8 chipID = 0; chipID < 2; chipID++)
	{
		size_t devID = _vdDevMap[0x02][chipID];  // 0x02 = YM2612 VGM chip type
		if (devID != (size_t)-1)
		{
			const CHIP_DEVICE& dev = _devices[devID];
			// Count channels if using GPGX or Gens core (both support scope)
			if (dev.base.defInf.devDef != NULL)
			{
				UINT32 coreID = dev.base.defInf.devDef->coreID;
				if (coreID == FCC_GENS || coreID == FCC_GPGX)
					count += 6;
			}
		}
	}

	// Count PSG channels (4 per chip) - MAME and Maxim cores support scope
	for (UINT8 chipID = 0; chipID < 2; chipID++)
	{
		size_t devID = _vdDevMap[0x00][chipID];  // 0x00 = SN76496/PSG VGM chip type
		if (devID != (size_t)-1)
		{
			const CHIP_DEVICE& dev = _devices[devID];
			if (dev.base.defInf.devDef != NULL)
			{
				UINT32 coreID = dev.base.defInf.devDef->coreID;
				if (coreID == FCC_MAXM || coreID == FCC_MAME)
					count += 4;
			}
		}
	}

	// Count AY8910 channels (3 per chip) - MAME and EMU2149 cores support scope
	for (UINT8 chipID = 0; chipID < 2; chipID++)
	{
		size_t devID = _vdDevMap[0x12][chipID];  // 0x12 = AY8910 VGM chip type
		if (devID != (size_t)-1)
		{
			const CHIP_DEVICE& dev = _devices[devID];
			if (dev.base.defInf.devDef != NULL)
			{
				UINT32 coreID = dev.base.defInf.devDef->coreID;
				if (coreID == FCC_MAME || coreID == FCC_EMU_)
					count += 3;
			}
		}
	}
...
const char* VGMPlayer::GetScopeChannelName(UINT8 channel) const
{
	// Static name buffers - enough for all possible scope channels
	// (2 YM2612 * 6 + 2 PSG * 4 + 2 AY * 3 = 26 max)
	static char s_names[26][16];

	// Count YM2612 channels to determine mapping
	UINT8 ym2612Count = 0;
	UINT8 ym2612ChipCount = 0;
	for (UINT8 chipID = 0; chipID < 2; chipID++)
	{
		size_t devID = _vdDevMap[0x02][chipID];
		if (devID != (size_t)-1)
		{
			const CHIP_DEVICE& dev = _devices[devID];
			if (dev.base.defInf.devDef != NULL)
	...
	// Count PSG channels
	UINT8 psgCount = 0;
	UINT8 psgChipCount = 0;
	for (UINT8 chipID = 0; chipID < 2; chipID++)
	{
		size_t devID = _vdDevMap[0x00][chipID];
		if (devID != (size_t)-1)
		{
			const CHIP_DEVICE& dev = _devices[devID];
			if (dev.base.defInf.devDef != NULL)
	...
	// Count AY8910 channels
	UINT8 ayChipCount = 0;
	for (UINT8 chipID = 0; chipID < 2; chipID++)
	{
		size_t devID = _vdDevMap[0x12][chipID];
		if (devID != (size_t)-1)
		{
			const CHIP_DEVICE& dev = _devices[devID];
			if (dev.base.defInf.devDef != NULL)
	...
	if (channel < ym2612Count)
	{
		UINT8 chipID = channel / 6;
		UINT8 ch = channel % 6;
		if (ym2612ChipCount > 1)
			snprintf(s_names[channel], sizeof(s_names[channel]), "YM2612 #%u Ch%u", chipID + 1, ch + 1);
		else
			snprintf(s_names[channel], sizeof(s_names[channel]), "YM2612 Ch%u", ch + 1);
		return s_names[channel];
	}

	if (channel < ym2612Count + psgCount)
	{
		UINT8 psgCh = channel - ym2612Count;
		UINT8 chipID = psgCh / 4;
		UINT8 ch = psgCh % 4;
		if (psgChipCount > 1)
			snprintf(s_names[channel], sizeof(s_names[channel]), "PSG #%u Ch%u", chipID + 1, ch + 1);
		else
			snprintf(s_names[channel], sizeof(s_names[channel]), "PSG Ch%u", ch + 1);
		return s_names[channel];
	}

	UINT8 ayCh = channel - ym2612Count - psgCount;
	UINT8 chipID = ayCh / 3;
	UINT8 ch = ayCh % 3;
	static const char* ayChNames[] = { "A", "B", "C" };
	if (ayChipCount > 1)
		snprintf(s_names[channel], sizeof(s_names[channel]), "AY #%u %s", chipID + 1, ayChNames[ch]);
	else
		snprintf(s_names[channel], sizeof(s_names[channel]), "AY %s", ayChNames[ch]);
	return s_names[channel];
}

UINT32 VGMPlayer::GetScopeData(UINT8 channel, float* buffer, UINT32 maxSamples) const
{
	if (!_scopeEnabled || buffer == NULL || maxSamples == 0)
		return 0;

	UINT32 writePos = 0;
	const float* scopeBuffer = NULL;
	UINT32 bufferSize = SCOPE_BUFFER_SIZE;

	// Count YM2612 channels to determine channel mapping
	UINT8 ym2612ChannelCount = 0;
	for (UINT8 chipID = 0; chipID < 2; chipID++)
	{
		size_t devID = _vdDevMap[0x02][chipID];
		if (devID != (size_t)-1)
		{
			...
			if (dev.base.defInf.devDef != NULL)
			{
				UINT32 coreID = dev.base.defInf.devDef->coreID;
				if (coreID == FCC_GENS || coreID == FCC_GPGX)
					ym2612ChannelCount += 6;
			}
		}
	}

	// Count PSG channels
	UINT8 psgChannelCount = 0;
	for (UINT8 chipID = 0; chipID < 2; chipID++)
	{
		size_t devID = _vdDevMap[0x00][chipID];
		if (devID != (size_t)-1)
	...
	// Try YM2612 first
	if (channel < ym2612ChannelCount)
	{
		// Map channel to YM2612 chip and channel
		// Channels 0-5 = first YM2612, 6-11 = second YM2612 (if present)
		UINT8 chipID = channel / 6;
		UINT8 chipChannel = channel % 6;
		...
		const CHIP_DEVICE& dev = _devices[devID];
		if (dev.base.defInf.dataPtr == NULL || dev.base.defInf.devDef == NULL)
			return 0;
		...
		{
			ym2612_* ym = (ym2612_*)dev.base.defInf.dataPtr;
			scopeBuffer = YM2612_GetScopeBuffer(ym, chipChannel, &writePos);
			bufferSize = SCOPE_BUFFER_SIZE;
		}
		else if (coreID == FCC_GPGX)
		{
			scopeBuffer = ym2612_get_scope_buffer(dev.base.defInf.dataPtr, chipChannel, &writePos);
			bufferSize = SCOPE_BUFFER_SIZE;
		}
	}
	else if (channel < ym2612ChannelCount + psgChannelCount)
	{
		// Try PSG - channels after YM2612
		UINT8 psgChannel = channel - ym2612ChannelCount;
		UINT8 chipID = psgChannel / 4;
		UINT8 chipChannel = psgChannel % 4;

		if (chipID >= 2)
			return 0;

		size_t devID = _vdDevMap[0x00][chipID];  // 0x00 = SN76496/PSG
		if (devID == (size_t)-1)
			return 0;
		...
		if (coreID == FCC_MAXM)
		{
			scopeBuffer = sn76489_get_scope_buffer(dev.base.defInf.dataPtr, chipChannel, &writePos);
			bufferSize = SCOPE_BUFFER_SIZE;
		}
		else if (coreID == FCC_MAME)
		{
			scopeBuffer = sn76496_get_scope_buffer(dev.base.defInf.dataPtr, chipChannel, &writePos);
			bufferSize = SCOPE_BUFFER_SIZE;
		}
	}
	else
	{
		// Try AY8910 - channels after YM2612 and PSG
		UINT8 ayChannel = channel - ym2612ChannelCount - psgChannelCount;
		UINT8 chipID = ayChannel / 3;
		UINT8 chipChannel = ayChannel % 3;

		if (chipID >= 2)
			return 0;

		size_t devID = _vdDevMap[0x12][chipID];  // 0x12 = AY8910
		if (devID == (size_t)-1)
			return 0;
		...
		if (coreID == FCC_MAME)
		{
			scopeBuffer = ay8910_get_scope_buffer(dev.base.defInf.dataPtr, chipChannel, &writePos);
			bufferSize = SCOPE_BUFFER_SIZE;
		}
		else if (coreID == FCC_EMU_)
		{
			scopeBuffer = EPSG_get_scope_buffer((EPSG*)dev.base.defInf.dataPtr, chipChannel, &writePos);
			bufferSize = SCOPE_BUFFER_SIZE;
		}
	}
	...
	// Copy from ring buffer, starting from oldest samples
	UINT32 samplesToCopy = (maxSamples < bufferSize) ? maxSamples : bufferSize;

	// Read from write position (oldest) to write position - 1 (newest)
	UINT32 readPos = (writePos - samplesToCopy) & (bufferSize - 1);
	for (UINT32 i = 0; i < samplesToCopy; i++)
	{
		buffer[i] = scopeBuffer[readPos];
