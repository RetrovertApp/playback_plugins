...
void PlayerA::SetScopeEnabled(bool enabled)
{
	_scopeEnabled = enabled;
	if (_player != NULL)
		_player->SetScopeEnabled(enabled);
}

bool PlayerA::GetScopeEnabled(void) const
{
	return _scopeEnabled;
}

UINT8 PlayerA::GetScopeChannelCount(void) const
{
	if (_player == NULL)
		return 0;
	return _player->GetScopeChannelCount();
}

const char* PlayerA::GetScopeChannelName(UINT8 channel) const
{
	if (_player == NULL)
		return NULL;
	return _player->GetScopeChannelName(channel);
}

UINT32 PlayerA::GetScopeData(UINT8 channel, float* buffer, UINT32 maxSamples) const
{
	if (_player == NULL || !_scopeEnabled)
		return 0;
	return _player->GetScopeData(channel, buffer, maxSamples);
}
